...
const recursionPattern = (int1, int2) => {
    ...
    return duplicatedArray.concat(duplicatedArray.slice(0, duplicatedArray.length - 1).reverse());
}
// -------------------------------------------------------------------------------------------------------
...
const filterLinks = (str) => {
    // write your code here
    const regex = /\b(www\.[a-zA-Z0-9]+\.[a-zA-Z0-9\.]+)/g;
...
const isPalindrome = (str) => {
    // write your code here
    const nonAlpRegex = /[^a-zA-Z0-9]/;
    let lowerCa = str.toLowerCase().split("");
    for (let i = 0; i < lowerCa.length; i++) {
        if (nonAlpRegex.test(lowerCa[i])) {
            lowerCa.splice(i, 1);
            i--;
        }
    }
    let completeArray = lowerCa.join('');
    let reversedArray = lowerCa.reverse().join('');
    return completeArray === reversedArray;
}
// -------------------------------------------------------------------------------------------------------
...
const samePattern = (str, arr) => {
    ...
    if (stri.length !== arr.length) {
        return false; 
    }

    for (let i = 0; i < stri.length; i++) {
        if (!obj[stri[i]]) {
            if (reverseObj[arr[i]] && reverseObj[arr[i]] !== stri[i]) {
                return false;
            }
            obj[stri[i]] = arr[i];
            reverseObj[arr[i]] = stri[i];
        } else if (obj[stri[i]] !== arr[i]) {
            return false;
        }
    }
    return true; 
